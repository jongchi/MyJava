...
select trunc(sysdate-to_date('2021/01/01', 'YYYY/MM/DD')) as "D+day"
from dual
;
...
select max(sal) as "급여 최고액", min(sal) as "급여 최저액", sum(sal) as "급여 총액", round(avg(sal)) as "평균 급여"
from emp
;
...
select deptno, count(deptno) as "사원 수"
from emp
group by deptno
;
...
select job, deptno as dno, decode(deptno, 10, sum(sal)) as "부서 10", decode(deptno, 20, sum(sal)) as "부서 20", decode(deptno, 30, sum(sal)) as "부서 30"
from emp
group by job, deptno
order by deptno
;
